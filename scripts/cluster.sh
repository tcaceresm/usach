...
#   sed 's/kcal\/mol  \[=(1)+(2)+(3)-(4)\]//g' ${SDF_DIR}/${LIGAND_NAME}2Clust.sdf > ${SDF_DIR}/${LIGAND_NAME}2Clust2.sdf

    # Output directory
    mkdir -p ${SDF_DIR}/cluster
    
    echo "Starting clustering with Biomol2Clust"
    python3 ${BIOMOL2CLUST_PATH}/main.py input=${SDF_DIR}/${LIGAND_NAME}2Clust.sdf output=${SDF_DIR}/cluster min_cluster_size=5
